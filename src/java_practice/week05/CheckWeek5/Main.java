package java_practice.week05.CheckWeek5;
...
//        카테고리가 여행인 책 제목 조회
        bookList.stream().filter(book -> book.getCategory().equals("여행"))
                .forEach(f-> System.out.println("카테고리가 여행인 책 제목은 "+ f.getBookName()));
        System.out.println();

        //가격이 16200원 이하인 책 제목 조회
        bookList.stream().filter(book -> book.getPrice()<=16200).forEach(f-> System.out.println("가격이 16200원 이하인 책 제목은 "+f.getBookName()));
        System.out.println();

        //책 제목에 "경제”라는 용어가 들어간 책 제목 조회
        bookList.stream().filter(book -> book.getBookName().contains("경제"))
                .forEach(f-> System.out.println("책 제목에 경제라는 용어가 들어간 책 제목은 "+ f.getBookName()));
        System.out.println();

        //가격이 가장 비싼 책 가격 조회
        double maxPrice = bookList.stream().mapToDouble(Book::getPrice).max().getAsDouble();
        System.out.println("가격이 가장 비싼 책 가격 조회 : "+ maxPrice);
        System.out.println();

        //카테고리가 IT인 책들의 가격 합 조회
        double sum = bookList.stream().filter(book -> book.getCategory().equals("IT")).mapToDouble(Book::getPrice).sum();
        System.out.println("카테고리가 IT인 책들의 가격 합 : "+sum);
        System.out.println();
        ...
        discountedBookList.stream().forEach(b-> System.out.println("할인된 책제목 : "+b.getBookName()+" ,할인 가격 :"+b.getPrice()+"\n"));
