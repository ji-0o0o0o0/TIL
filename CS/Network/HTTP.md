...
<details>
    <summary>정답</summary>
A. GET은 데이터를 URL에 담아 사용하고, POST는 데이터를 Body에 담아 전송/생성할때 사용</br>
...
## 3. REST API
`REST API = URI로 자원표현 + HTTP Method로 행위 표현`
### REST(Representational State Transfer)
- HTTP 기반으로 자원을 URI로 표현하고 HTTP Method로 행위를 표현하는 설계 방식
...
#### ? 용어정리
- URI vs URL
    - URI : 자원을 **식별**하는 것(더 넓은 개념)
        - 경로만 있어도 됨 
    - URL : 자원의 **위치**를 나타내는 것(URI의 부분 집합)
        -  https:// 같은 프로토콜 + 도메인 + 경로가 다 포함된 전체 주소
    ```
    URI ⊃ URL

    예)
    /users/1          → URI (자원 식별)
    https://api.com/users/1 → URL (위치까지 포함)
    ```
...
<details>
    <summary>정답</summary>
    A. URI로 자원을 표현하고 HTTP Method로 행위를 표현하는 설계 방식<br>
    URI는 명사를 사용, 행위는 GET/POST/PUT/DELETE 등으로 표현
</details>

### 💡 실무 연결
- 실무에서 /getUser 대신 GET /users 로 설계하는 이유가 REST 원칙 때문